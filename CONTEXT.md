...
| ----------------------------- | ------------- | ----------- | ----------------------------------------------------------- |
| Проект                        | проєкт        | project     | «проект» (устаревшее написання без «є»)                     |
| Команда (`teams`)             | команда       | team        | —                                                           |
| Роль SENIOR                   | сеньйор       | senior      | «синьйор» (італ. форма звертання «пан», не грейд)           |
| Роль JUNIOR                   | джуніор       | junior      | «джун» (сленг у формальному списку ролей)                   |
| Роль DROP                     | дроп          | drop        | —                                                           |
| Роль ACCOUNTANT               | бухгалтер     | accountant  | —                                                           |
| Роль HR                       | HR            | HR          | —                                                           |
| Роль ADMIN                    | адміністратор | admin       | —                                                           |
| Валидация дохода (доход)      | дохід         | income      | «прихід», «прибуток» (три слова на одне поняття цієї хвилі) |
| Заявка на выплату (выплата)   | виплата       | payout      | —                                                           |
| Обязательство                 | зобов’язання  | obligation  | «начислення»                                                |
| Доля дропа / доля синьора     | частка        | share       | «Ставка» (термін зі списку `_Избегать_` вище, «Доля дропа») |
...
не `...`. `en`-типографика — прямые кавычки `"…"`, не `«…»` (ёлочки — это калька с кириллицы).
