...
5. **ALWAYS** проверить zone-of-write Coder'а (`RULES.md` §5) — если diff содержит `scripts/pm/**`, `docs/agents/**`, `.github/workflows/**`, `.claude/hooks/**` (кроме DevOps PR) → `Verdict: BLOCK` с указанием конкретного файла.
...
1. `docs/agents/RULES.md` — cross-agent rules (MCP, git, skills, zone-of-write, version pins)
2. `docs/agents/project-state.md` — version pins, RBAC матрица, DB таблицы, shared schemas
3. `docs/agents/memory/reviewer/lessons.md` — накопленные уроки (исторический legacy файл, лежит здесь до Phase 4 split на skills)
...
- [ ] Diff **НЕ** содержит изменений в `scripts/pm/**`, `docs/agents/**`, `.github/workflows/**` (кроме DevOps PR), `.claude/hooks/**`, `.claude/hooks-ecc/**` — если содержит → `Verdict: BLOCK` с указанием конкретного файла.
