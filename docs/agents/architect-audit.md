...
**Scope:** `docs/agents/**` + корневой `CLAUDE.md`
...
`docs/agents/**` содержит **23 .md файла** (`233992` bytes ≈ **228 KB**). Корневой `CLAUDE.md` — ещё `36 KB`. Итого **264 KB** только в multi-agent layer.

### 1.1. Top-level agent docs

| Файл                             | Lines | Size  | Назначение (сейчас)                                                              |
| -------------------------------- | ----- | ----- | -------------------------------------------------------------------------------- |
| `docs/agents/coder.md`           | 580   | 34 KB | Полный system prompt Coder: workflow, MCP, skills, watchdog, zone-of-write       |
| `docs/agents/pm.md`              | 410   | 24 KB | Полный system prompt PM: 4 режима, Mode 2.A-2.E, dispatch decision, User Testing |
| `docs/agents/pm-snippets.md`     | 411   | 20 KB | On-demand сниппеты Agent()/gh/E2E/recovery (загружается через skill)             |
| `docs/agents/reviewer.md`        | 300   | 20 KB | Code Review: workflow, security check, AST patterns, write-then-post resilience  |
| `docs/agents/autotest.md`        | 289   | 16 KB | Test agent: 3 режима, AC-first, RBAC coverage, anti-no-op verification           |
| `docs/agents/CLAUDE-pm.md`       | 326   | 16 KB | PM notes: GH secrets, ScheduleWakeup limitations, pm-state schema v2             |
| `docs/agents/ba.md`              | 260   | 16 KB | BA workflow: коллизии, requirements, brief, role boundaries                      |
| `docs/agents/CLAUDE-devops.md`   | 224   | 16 KB | DevOps notes: pipeline architecture, secrets, branch protection                  |
| `docs/agents/devops.md`          | 230   | 12 KB | DevOps workflow: tasks, blockers, Docker, GHA conventions                        |
| `docs/agents/CLAUDE-tools.md`    | 195   | 12 KB | Полный справочник MCP/native tools                                               |
| `docs/agents/CLAUDE-autotest.md` | 118   | 8 KB  | AutoTest notes: seed users, anti-patterns, селекторы                             |
| `docs/agents/CLAUDE-ba.md`       | 101   | 8 KB  | BA notes: бизнес-модель, role table, pipeline diagram                            |
| `docs/agents/CLAUDE-coder.md`    | 93    | 8 KB  | Coder notes: команды, структура, статус фаз, технические gotchas                 |
| `docs/agents/CLAUDE-reviewer.md` | 75    | 4 KB  | Reviewer notes: canonical architecture, version pins, RBAC, inline-comments      |

### 1.2. Memory (lessons) и meta

| Файл                                     | Lines | Size | Содержание                                                |
| ---------------------------------------- | ----- | ---- | --------------------------------------------------------- |
| `docs/agents/memory/README.md`           | 79    | 8 KB | Lessons format, priority tags (P0/P1/P2), rotation policy |
| `docs/agents/memory/pm/lessons.md`       | 18    | 8 KB | 11 строк уроков PM                                        |
| `docs/agents/memory/coder/lessons.md`    | 17    | 8 KB | 10 строк уроков Coder                                     |
| `docs/agents/memory/reviewer/lessons.md` | 16    | 4 KB | 3 строки уроков + HTML-comment placeholder                |
| `docs/agents/memory/autotest/lessons.md` | 14    | 4 KB | 6 строк уроков AutoTest                                   |
| `docs/agents/memory/devops/lessons.md`   | 12    | 4 KB | 5 строк уроков DevOps                                     |
| `docs/agents/archive/qa.md`              | 174   | 8 KB | Archived QA agent (упразднён)                             |
| `docs/agents/archive/CLAUDE-qa.md`       | 58    | 4 KB | Archived QA notes                                         |
...
Использован `grep -rn "docs/agents" docs/agents/**/*.md`. Markdown link targets — не AST-конструкции, поэтому ast-grep здесь хуже plain grep.

### 2.1. Ссылки между .md (адресные пары)

```
ba.md
  → docs/agents/CLAUDE-ba.md (обязательное чтение)

autotest.md
  → docs/agents/CLAUDE-tools.md
  → docs/agents/CLAUDE-autotest.md
  → docs/agents/memory/autotest/lessons.md
  → docs/agents/autotest.md  (self-reference)

coder.md
  → docs/agents/CLAUDE-tools.md
  → docs/agents/CLAUDE-coder.md
  → docs/agents/memory/coder/lessons.md
  + zone-of-write упоминает docs/agents/** как off-limits

pm.md
  → docs/agents/CLAUDE-tools.md (`CLAUDE-tools.md` linked)
  → docs/agents/CLAUDE-pm.md
  → docs/agents/memory/pm/lessons.md
  → docs/agents/pm-snippets.md
  → CLAUDE-pm.md → секция «ScheduleWakeup limitations»

devops.md
  → docs/agents/CLAUDE-tools.md
  → docs/agents/CLAUDE-devops.md
  → docs/agents/memory/devops/lessons.md

reviewer.md
  → docs/agents/CLAUDE-tools.md
  → docs/agents/CLAUDE-reviewer.md
  → docs/agents/memory/reviewer/lessons.md

pm-snippets.md
  → docs/agents/coder.md
  → docs/agents/CLAUDE-coder.md
  → docs/agents/memory/coder/lessons.md
  → docs/agents/autotest.md
  → docs/agents/memory/autotest/lessons.md
...
1. docs/agents/CLAUDE-tools.md
2. /.clauderules
3. docs/agents/CLAUDE-<role>.md
4. docs/agents/memory/<role>/lessons.md
...
`docs/agents/CLAUDE-pm.md`, `CLAUDE-ba.md`, `CLAUDE-devops.md`, `ba.md` ссылаются на `.github/workflows/coder.yml`, `autotest.yml`, `devops.yml`, `ai-review.yml` как на **активные**. Реальность:
...
`docs/agents/archive/qa.md` существует, но его никто не должен читать. Сейчас в archive только QA — нет structure для будущих архивов.
...
- ⛔ Нет **CHANGES.md** / changelog в `docs/agents/`
- ⛔ Нет **deprecation markers** в стейл-секциях
- ⛔ Нет общего файла **`docs/agents/common-rules.md`** для cross-agent правил (commit hygiene, MCP, etc.)
- ⛔ Нет **task spec templates inventory** в `docs/specs/tasks/templates/` — упомянуто в `pm.md` line 68 что должно быть, но не верифицировано
...
Design doc `docs/agents/architecture-v2.md` должен решить:
...
См. `docs/agents/architecture-v2.md`.
