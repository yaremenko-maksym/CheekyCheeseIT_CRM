...
flowchart TD
    ...
    BA -->|docs/specs/pm-brief.md| PM[PM]
...
BA → PM: docs/specs/pm-brief.md (commit + push)
...
Любой агент:
  1. Read docs/agents/<self>.md → Golden rules + Recovery checklist
  2. Read docs/agents/RULES.md → cross-agent rules
  3. Read docs/agents/project-state.md → текущие фазы
  4. Read docs/agents/memory/<self>/lessons.md

PM additional:
  5. Read docs/specs/pm-state.json (если есть)
  6. tail -5 .claude/coder-activity.log
  7. ls docs/specs/tasks/*.blocked.md
  8. ls docs/specs/tasks/*.progress.md (для крупных задач)
  9. Sync с remote: git fetch origin
 10. Resume on next_action (если есть и scheduled_at < now)

Coder additional:
  5. cat docs/specs/tasks/<my-task>.progress.md (если есть)
...
| 8.2   | Semantic milestones (`<task>.progress.md`)              | Файл в `docs/specs/tasks/` (committed) | «Какой milestone reached»                  |
...
| ---------------------------------------------- | -------------------------------------------------- | ----------------------------------------------------------------- |
| Coder обнаружил неописанную бизнес-логику      | Coder создаёт `docs/specs/tasks/<task>.blocked.md` | PM читает на next wakeup → задаёт USER                            |
