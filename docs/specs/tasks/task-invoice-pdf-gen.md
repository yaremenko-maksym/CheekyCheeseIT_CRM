...
```
Запустить через `ts-node` и открыть PDF в Preview. Если Cyrillic читается + QR работает → success.
3. Commit: `feat(invoice): PDF generation service с Cyrillic font + QR verification` + `ac_verified: 1-9`
4. Push → PR с label `ai-review-ready`
