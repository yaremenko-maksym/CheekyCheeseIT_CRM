...
> **Связанные файлы:**
>
> - `docs/specs/pm-state.json` — live state file (gitignored, не редактируется руками)
> - `docs/agents/pm.md` — Mode 2 monitoring + Aggregate verdict logic + Critical-path trigger zones
> - `docs/agents/pm-snippets.md` — секция «pm-state.json schema v2» + Event types
>
...
```json
{ "at": "...", "type": "brief_approved", "brief": "docs/specs/pm-brief.md" }
```
...
```json
{ "at": "...", "type": "task_file_created", "file": "docs/specs/tasks/task-<slug>.md" }
```
...
```json
{
  ...
  "type": "agent_started",
  "agent": "coder",
  "task_file": "docs/specs/tasks/task-<slug>.md"
}
```
...
| Initial (2026-05-XX)    | Baseline event types — see git log `docs/specs/pm-state.json`.                                                                                                                                                                                                                                                                                                                                         |

---

## Reference

- `docs/agents/pm.md` — Mode 2 monitoring (event-to-action mapping), Critical-path trigger zones, Aggregate verdict logic, Mode 2.F (timeout handler)
- `docs/agents/pm-snippets.md` — pm-state.json schema v2 + agent dispatch snippets + типичные длительности
- `docs/agents/code-reviewer.md` — code-reviewer system prompt (default reviewer)
- `docs/agents/security-reviewer.md` — security-reviewer system prompt (critical-path)
- `docs/agents/contracts.md` — Reviewer verdict semantics (§6) + labels lifecycle (§2)
