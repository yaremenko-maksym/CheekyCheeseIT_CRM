...
#
# Бери из этого списка сам хеш (7-40 символов). Тег `main` виден там же, но
# это не отдельная версия для отката, а ярлык на САМЫЙ СВЕЖИЙ билд (деплой
# переставляет его на каждый пуш) — то есть, скорее всего, тот самый билд,
# от которого откатываемся.
...
4. Если оба образа найдены — `write-env`/`copy-compose`/`deploy` идут как
   на обычном деплое; `docker compose pull` на VPS вытягивает уже
   существующий образ под этим тегом, ничего не пересобирая.
