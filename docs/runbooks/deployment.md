...
**Основной способ — `workflow_dispatch` с `image_tag`.** С task-infra-rollback-
deploys-not-rebuilds (эта правка) это ЧЕСТНЫЙ откат: workflow разворачивает
уже опубликованный в GHCR образ под указанным тегом и **не пересобирает код,
не перезаписывает ничей тег**. До этой правки было наоборот — ввод старого
SHA всё равно пересобирал текущий `main` HEAD и публиковал результат ПОД
СТАРЫМ ТЕГОМ, то есть уничтожал сам образ, к которому пытались откатиться,
молча, в момент аварии.

Единственный путь на прод для владельца — этот workflow: SSH к серверу у
владельца нет (см. ниже раздел "Экстренный SSH-путь").

```bash
# Список доступных тегов (git SHA) в GHCR:
# GitHub → Packages → cheekycheeseit-crm-api → версии
#
# Бери из этого списка сам хеш (7-40 символов) — И БЕРИ ЕГО РОВНО ТАКИМ,
# КАКИМ ЕГО ПОКАЗЫВАЕТ PACKAGES (copy-paste, не пересчитывай на глаз).
# Причина не косметика: короткий git-хеш — переменной длины (обычно 7
# символов, но git сам удлиняет его до 8+, если репозиторий разросся
# настолько, что 7 стало неоднозначно) — то, что где-то в логах промелькнуло
# как 8-символьный хеш, в реестре может значиться под 7-символьным тегом.
# Тег `main` виден там же, но это не отдельная версия для отката, а ярлык
# на САМЫЙ СВЕЖИЙ билд (деплой переставляет его на каждый пуш) — то есть,
# скорее всего, тот самый билд, от которого откатываемся.

# Задеплоить конкретный тег вручную через workflow_dispatch:
# GitHub → Actions → Deploy → Run workflow → image_tag: <git-sha>
```

Что происходит внутри `build`-джобы на этом прогоне (в порядке шагов):

1. **Валидация формы.** `main` или что угодно, кроме 7-40 hex-символов, —
   явная ошибка шага ("image_tag input is not a valid git SHA") ДО чего-либо
   ещё — security review PR #613 round 2, SR-M-1. Подставляй хеш из списка
   Packages выше.
2. **Build & push для API и nginx ПРОПУСКАЮТСЯ целиком.** Ничего не
   собирается, тег `<git-sha>` не перезаписывается — шаги несут
   `if: inputs.image_tag == ''`, на этом прогоне оба false.
3. **"Verify rollback image exists in GHCR"** проверяет манифест ОБОИХ
   образов (`-api` и `-nginx`) под запрошенным тегом
   (`docker buildx imagetools inspect`, без скачивания слоёв). Нет образа
   под этим тегом — явный `::error::` и job падает ЗДЕСЬ, до
   `write-env`/`copy-compose`/деплоя на VPS. Ничего не отправляется на
   сервер, пока оба образа не подтверждены.
4. **"Verify rollback image fingerprint"** (security review PR #615 round 2
   SR-M-1, round 3 SR-M-4) читает `GIT_COMMIT`, запечённый В КОНФИГ ОБОИХ
   образов — API и nginx (не в слои — тот же дешёвый manifest-only запрос,
   что и шаг 3), и сверяет каждый с запрошенным тегом. Совпало — идём
   дальше. Не совпало — job падает ЗДЕСЬ: тег указывает не на тот коммит,
   каким назвался (типичный след старого бага "откат пересобирает" — см.
   предисловие этого раздела). У образов, собранных ДО этого PR, такого
   отпечатка нет вовсе — это НЕ отказ, а `::notice::` "не могу проверить", и
   деплой идёт дальше на одной только проверке существования (шаг 3). Это
   ДРУГОЕ, чем "не смог прочитать манифест вообще" (например, будущий
   мультиплатформенный билд) — та ситуация даёт отдельный, громкий
   `::warning::`, а не тихо сливается с "отпечатка нет" (round 3 SR-M-4:
   раньше сливалась — код возврата docker в этом случае 0, а значение
   пустое, то есть неотличимо от честного отсутствия поля без отдельной
   проверки).
5. **"Verify rollback target commit has all hard-required files"**
   (security review PR #615 round 3, SR-H-3) чекаутит полную историю и
   проверяет — БЕЗ обращения к VPS, чисто через `git cat-file` — что ЗАПРОШЕННЫЙ
   КОММИТ ОТКАТА реально содержит все файлы, которые скрипт джобы `deploy`
   считает безусловно обязательными (compose-файлы + миграции, помеченные
   там `UNCONDITIONAL`). До этой проверки то же самое обнаруживалось только
   ГЛУБОКО внутри `deploy`-джобы, УЖЕ ПОСЛЕ того, как `write-env` (соседняя,
   не следующая джоба — обе несут только `needs: build`, выполняются
   ПАРАЛЛЕЛЬНО) успевала переписать `.env.production` на VPS, и уже после
   того, как `copy-compose` успевала скопировать на VPS всё, что при этом
   коммите СУЩЕСТВУЕТ (только 4 самых свежих безусловных файла в конце
   ~25-шаговой джобы реально проверяют своё присутствие — падением самого
   `scp`, а не явной проверкой). Падает здесь — значит `write-env` и
   `copy-compose` вообще не стартуют (обе `needs: build`), и на VPS
   действительно ничего не уходит. См. следующий подраздел про то, как
   узнать ЗАРАНЕЕ, докуда сегодня можно откатиться этим шагом, не дожидаясь
   отказа.
6. Если всё выше прошло — `write-env`/`copy-compose`/`deploy` идут как на
   обычном деплое, но с ОДНИМ отличием от автоматического пути:
   `copy-compose` чекаутит репозиторий не на текущий `main`, а на
   **запрошенный коммит отката** — см. следующий подраздел, это важно.

### Compose-файлы и DDL на откате берутся с коммита отката, не с `main`

**Security review PR #615 round 2, SR-H-2.** До этого фикса джоба
`copy-compose` чекаутила репозиторий БЕЗ указания ревизии — на откате это
означало, что compose-файлы и SQL-миграции копировались с СЕГОДНЯШНЕГО
`main`, а не с того коммита, на который откатывались. Итог: старый код
(запрошенный образ) разворачивался поверх миграций, которые этот код
никогда не видел — молча, на единственном пути, который существует
специально для восстановления после аварии.

Теперь `copy-compose` на откате явно переключается на коммит, который
СООТВЕТСТВУЕТ запрошенному тегу образа. Практическое следствие:

- Миграции, добавленные ПОСЛЕ коммита отката, на VPS не попадают вообще —
  соответствующие DDL-файлы просто отсутствуют в чекауте, и уже существующие
  guard-проверки ("не смержено ещё") тихо их пропускают. Ничего нового не
  применяется поверх старого кода.
- Если коммит отката СТАРШЕ, чем момент, когда в `main` появился один из
  "безусловных" миграционных файлов (в скрипте деплоя они помечены
  `UNCONDITIONAL` — файл жёстко ожидается на месте, `exit 1` если его нет),
  откат отказывает. **Security review PR #615 round 3, SR-H-3, исправляет
  ГДЕ именно:** раньше это обнаруживалось в самом конце `copy-compose`
  (после того, как компоуз-файлы, восемь более старых миграций и секреты в
  `.env.production` уже успевали уйти на VPS — см. предыдущий подраздел,
  шаг 5) — то есть заявление "раньше, чем что-либо уйдёт на VPS" было
  ЧЕСТНЫМ намерением, но не фактом. Теперь эта проверка — отдельный шаг
  `build`-джобы (предыдущий подраздел, шаг 5), выполняется ДО того, как
  `write-env`/`copy-compose` вообще стартуют, и заявление стало верным.
  Сам отказ остаётся ЧЕСТНЫМ, не багом: скрипт деплоя не умеет
  разворачивать откат настолько далеко назад, и говорит об этом прямо, а не
  разворачивает несогласованное состояние.

  **Как узнать СЕГОДНЯШНИЙ предел ДО того, как пробовать откат** (а не
  только по факту отказа) — одна команда, дающая коммит, старше которого
  откатываться этим workflow уже нельзя:
  ...
  из перечисленных файлов — то есть коммит, добавивший последний по времени
  "безусловный" файл. Любой тег отката СТАРШЕ этого коммита откат не
  пройдёт (упадёт на шаге 5 из предыдущего подраздела, до VPS); РОВНО этот
  коммит и любой более новый — безопасны. Список путей — сознательное
  зеркало (не вывод, не импорт) файлов в `build`-джобы шаге "Verify rollback
  target commit has all hard-required files" (deploy.yml) — если тот список
  меняется (появляется новый безусловный файл), эта команда должна
  измениться тем же PR, иначе она снова начнёт врать про предел, только уже
  в другую сторону. **Это больше не держится только на этом абзаце** (PR
  #615, раунд 4): `scripts/devops/check-prod-ddl-wiring.py` сравнивает два
...
- **Схема при этом НЕ откатывается назад — и в общем случае не может.**
  ...
  миграция (переопределение вьюхи, удаление колонки), эти изменения УЖЕ
  применились к живой базе к тому моменту, как было принято решение
  откатываться — часовым поясом раньше самого отката. Пиннинг чекаута
  останавливает применение ЧЕГО-ЛИБО ДАЛЬНЕЙШЕГО в ЭТОМ прогоне; он не
  отменяет то, что уже сделала предыдущая (плохая) миграция. Это
  неизбежная асимметрия отката — назвать её честнее, чем делать вид, что
  её нет.

### После отката — обязательный следующий шаг: откат в `main`

**Security review PR #615 round 2, SR-H-1.** Этот workflow НЕ трогает
`main` — он только разворачивает уже собранный образ. Значит откат
**временный по конструкции**: следующий мерж в `main` (авто-деплой) или
ближайшее воскресенье 03:00 UTC (еженедельный пересобор) снова развернут
`main` HEAD и молча отменят откат — возможно, ночью, когда никто не
смотрит.

**Обязательный следующий шаг после отката, не примечание:** откати сам
коммит(ы) в `main` (`git revert <плохой-sha>`, открой PR, смерджи), чтобы
следующий автоматический деплой УЖЕ БЫЛ тем состоянием, которое нужно.
Пока это не сделано — прод держится на честном слове расписания, а не на
твоём решении.

Это же предупреждение продублировано в двух местах самого прогона (по
конструкции, не по недосмотру): `::notice::` в логе `build`-джобы (видно
сразу при запуске, до ожидания остального пайплайна) и отдельный блок в
`$GITHUB_STEP_SUMMARY` job'ы `deploy` (видно на странице прогона без
пролистывания многосотстрочного SSH-лога, и остаётся видимым после того,
как сам лог прокрутился из внимания).

### Проверка после отката — как убедиться, что реально откатилось

`GET /api/health` отдаёт build-fingerprint (`commit`,
`apps/api/src/health/health.controller.ts`) — сверь его с запрошенным
тегом отката тем же способом prefix-match, что и шаг 4 выше (короткий и
полный хеш — не обязательно одной длины, один всегда префикс другого).

**Честная оговорка:** для образов, собранных ДО того, как build-arg
`GIT_COMMIT` появился в `apps/api/Dockerfile` (security review PR #613),
`commit` в ответе будет `"unknown"` — то есть для части сегодняшней
популяции существующих образов эта проверка отвечает "не могу сказать", а
не "да"/"нет". В этом случае единственный способ убедиться — job summary
самого прогона (шаг выше) плюс то, что шаги 3-4 build-джобы вообще дошли
до конца, не упав.

### Экстренный SSH-путь (не для владельца — нет SSH-ключа к VPS)

Тот же результат вручную, для держателя `VPS_SSH_KEY` (обычно только сам
деплой-пайплайн). Использовать только если сам workflow недоступен (например,
GitHub Actions лежит):

**Security review PR #615 round 2, LOW-1:** `docker-compose.ghcr.yml` резолвит
образ как `...:${IMAGE_TAG:-main}` — БЕЗ явного `IMAGE_TAG` в окружении и БЕЗ
`--env-file .env.production` (единственное место, откуда `docker compose`
подхватит его сам — файл называется не `.env`, автозагрузки не будет) команда
молча уедет на тег `main`, то есть на ТОТ САМЫЙ битый билд, от которого
пытаешься уйти. Обе команды ниже несут `IMAGE_TAG=<старый-sha>` ПЕРЕД
`docker compose` намеренно — не сокращай их до голого `docker compose up -d`.
