...
4. **"Verify rollback image fingerprint"** (security review PR #615 round 2,
   SR-M-1) читает `GIT_COMMIT`, запечённый В КОНФИГ API-образа (не в слои —
   тот же дешёвый manifest-only запрос, что и шаг 3), и сверяет с
   запрошенным тегом. Совпало — идём дальше. Не совпало — job падает
   ЗДЕСЬ: тег указывает не на тот коммит, каким назвался (типичный след
   старого бага "откат пересобирает" — см. предисловие этого раздела). У
   образов, собранных ДО этого PR, такого отпечатка нет вовсе — это НЕ
   отказ, а `::notice::` "не могу проверить", и деплой идёт дальше на
   одной только проверке существования (шаг 3).
5. Если всё выше прошло — `write-env`/`copy-compose`/`deploy` идут как на
...
- Если коммит отката СТАРШЕ, чем момент, когда в `main` появился один из
  ...
  джоба `copy-compose` **упадёт с явной ошибкой** на шаге копирования этого
  файла — раньше, чем что-либо уйдёт на VPS. Это ЧЕСТНЫЙ отказ, не баг:
  скрипт деплоя не умеет разворачивать откат настолько далеко назад, и
  говорит об этом прямо, а не разворачивает несогласованное состояние. На
  практике это ограничивает, насколько далеко можно откатиться этим же
  workflow — самый недавний такой коммит и есть предел.
