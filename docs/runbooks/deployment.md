...
#
# Бери из этого списка сам хеш (7-40 символов) — оба способа ниже рассчитаны
# именно на него. Тег `main` виден там же, но это не отдельная версия для
# отката, а ярлык на САМЫЙ СВЕЖИЙ билд (деплой переставляет его на каждый
# пуш) — то есть, скорее всего, тот самый билд, от которого откатываемся.

# Задеплоить конкретный тег вручную через workflow_dispatch:
# GitHub → Actions → Deploy → Run workflow → image_tag: <git-sha>
# `main` здесь workflow отклонит явной ошибкой шага ("image_tag input is not
# a valid git SHA") прежде, чем что-либо соберёт или задеплоит — security
# review PR #613 round 2, SR-M-1. Подставляй хеш из списка выше.
