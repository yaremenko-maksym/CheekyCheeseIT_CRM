...
| `SKIPPED`, `skip_reason`          | письма и не должно было быть: `NO_ADDRESS` — шаг 1 пропущен, `CHANNEL_OFF` — тип выключен в настройках, `USER_ARCHIVED` — учётная запись архивирована, `STALE` — согласование/контракт уже неактуальны (отозвано, пересоздано, решено, подписано); тот же резолвер, что видит попап уведомлений (бэклог 208) |
