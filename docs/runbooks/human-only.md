...
| `FAILED`                 | провайдер отказал — код статуса в `last_error`, адреса там нет намеренно. Если в `last_error` не `HTTP …`, а фраза с именем функции — это сбой в нашем коде, письмо не уходило: нести текст разработчику как есть                                          |
