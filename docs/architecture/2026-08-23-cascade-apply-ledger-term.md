...
### 1.7. Ветка переплаты: `amount` PAID-строки НЕ обновляется (Confidence: HIGH) — обязательно к соблюдению

AC3 основного ADR: при `новая_доля ≤ settled_amount` строку в `PENDING` не возвращаем, она
остаётся `PAID`. Вопрос: обновлять ли `transactions.amount` этой строки.
...
- **Что при этом НЕ ломается.** Откат drop-производной каскад делает штатно, терм 9 держит
  леджер точным, «уже выплачено» видно. Не работает ровно одно: закрыть остаток через систему.
  Строка ждёт задачи 3b, а не теряется.
