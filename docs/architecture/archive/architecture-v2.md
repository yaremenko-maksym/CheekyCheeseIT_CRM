...
- `.claude/agents/common-rules.md` — cross-agent правила (commit hygiene, MCP order, skills full table, session-recovery long form)
- `.claude/agents/project-state.md` — снимок проекта (миграции, фазы, версии). Single source of truth, обновляется PM/BA. Все agent docs ссылаются.
...
6. RESPECT zone-of-write — see common-rules.md#zone-of-write-contract for full table; Coder may edit only apps/api, apps/web, apps/e2e, packages/, .claude/tasks/<my-task>.progress.md, .blocked.md
...
3. Append в .claude/agents/memory/<agent>/lessons.md
...
Новый файл: `.claude/agents/contracts.md`. Содержит **формализованный state-machine** взаимодействия агентов.
...
PM (any agent):
  1. Read .claude/agents/<self>.md → Golden rules + Session-recovery
  2. Read pm-state.json (if exists)
  3. tail -5 .claude/coder-activity.log
  4. ls .claude/tasks/*.blocked.md
  5. ls .claude/tasks/*.progress.md (для крупных задач)
  6. Sync с remote: git fetch origin
  7. Resume on next_action (если есть)

Coder (after compaction):
  1. Read .claude/agents/coder.md → Golden rules
  2. cat .claude/tasks/<my-task>.progress.md (если есть)
...
### 9.1. `.claude/agents/CHANGES.md` (новый)
...
- Update README.md в `.claude/agents/`
...
- Update CLAUDE.md (top-level pointer на новую структуру .claude/agents/)
...
6. **Archive старых лессонов — `archive.md` в той же папке или общая `.claude/agents/archive/`?**
