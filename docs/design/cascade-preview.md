...
- для **семейства синьора** — `SENIOR_PENDING_PAYOUT` И `SENIOR_INCOME` — переиспользовать **уже
  переданный** `tx.receiverName` (источник — `ADMIN_INCOME`/`SENIOR_INCOME`, чей получатель И ЕСТЬ
  синьор) — подпись «Синьору {tx.receiverName}»;
- для **семейства дропа** — `DROP_PENDING_PAYOUT` И `PAYOUT_DROP` — имя недостижимо из уже
  загруженных данных, подпись ограничивается «Доля дропа» (без имени) — честно, не выдумывать.

> **Правка, 2026-08-24 (UX-1, fidelity-аудит).** Раньше здесь стояли только «ожидающие» типы —
> `SENIOR_PENDING_PAYOUT` / `DROP_PENDING_PAYOUT`. Это ошибка **спеки**, а не только реализации:
> `settleByCompany` флипает IOU **на месте** (`SENIOR_PENDING_PAYOUT` → `SENIOR_INCOME`,
> `DROP_PENDING_PAYOUT` → `PAYOUT_DROP`), а `loadCascadeSnapshot` кладёт в план `type: d.type`,
> то есть **текущий** тип. Совпадение только с ожидающим типом промахивалось ровно по той
> популяции, ради которой экран и делается: уже оплаченная доля, которая вот-вот вернётся в
> ожидание выплаты, показывалась без получателя. Найдено живым прогоном каскада, не чтением.
...
> **Правка, 2026-08-25 (QA-H-1, manual QA).** Контракт производной несёт **две** денежные
> величины, а не одну, и таблица выше описывала прежнюю модель:
>
> | Поле              | Что это                                                                                                              | Кто читает                                                            |
> | ----------------- | -------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------- |
> | `newAmount`       | что строка **будет держать** после сохранения — доля, прижатая полом к уже выплаченному (`floorAmountAtAccumulator`) | колонка «Стало», «К доплате»                                          |
> | `recomputedShare` | во что **пересчиталась доля** до пола                                                                                | текст `OVERPAYMENT`, предикат переплаты, журнал `CASCADE_OVERPAYMENT` |
>
> Разделение вынужденное, а не косметическое: пол **уничтожает** сырую долю, а её потребляют три
> места. Пока величина была одна, «Стало» показывало число, которое система уже решила не
> записывать — manual QA замерил дампом БД: на экране `100 → 50`, в строке `100`.

> **Правка, 2026-08-24 (UX-7, fidelity-аудит раунд 4).** Весь блок «что можно показать сегодня
> **без нового API-поля**» — включая «для `DROP_PENDING_PAYOUT` … ограничивается «Доля дропа» (без
> имени) — честно, не выдумывать» — **устарел**. Поле добавлено: `CascadeDerivativePlan.receiverName`
> резолвится сервером для **каждой** производной (§14.2 предлагала именно это, предложение
> реализовано). Актуальное правило: подпись берётся из `derivative.receiverName` — «Синьору {имя}»
> для семейства синьора, «Дропу {имя}» для семейства дропа; безымянный фолбэк «Доля дропа»
> сохраняется только когда получателя нет в данных (выдумывать по-прежнему нельзя).
>
> **Как это вскрылось и почему стоит запомнить.** Поле добавили в раунде 3, а до экрана довели
> только ветку синьора — то есть асимметрию, которую §14.2 сама называет нежелательной, мы
> описали, а потом воспроизвели. Правка контракта не считается доехавшей, пока её не прочитали
> **все** ветки, которые её касаются.

> **Правка примера, 2026-08-24 (SPEC-M-1 / COPY-L-1).** Раньше здесь стояли две фразы с точкой
> («…она не редактируется. Ошибку исправляйте сторнирующей транзакцией.»), и тут же — инструкция
> «тот же регистр, что у четырёх принятых». Это противоречило само себе: все четыре сообщения в
> `CASCADE_LEDGER_FACT_MESSAGES` — **одно предложение, средство после запятой, без точки в конце**.
> Реализация следовала форме, спека — своему примеру; правой оказалась реализация, и пример
> приведён к ней. Доводы (copy-review): строки рендерятся в двух разных обёртках (баннер панели и
> узкая полоса ошибки диалога), и строка без терминальной точки одинаково ложится в обе; точка у
> двух из шести сообщений, показываемых по одному в одном и том же баннере, читается как «говорят
> две разные системы». Форма «средство отдельным предложением» сама по себе не хуже, но допустима
> только для всех шести сразу, а четыре из них живут в `@crm/shared` и делятся с ответом API —
...
> **Правка, 2026-08-24 (UX-3 fidelity + COPY-M-2 copy-review).** Два уточнения к формуле выше.
> (1) Вторая половина подписи называется **«к доплате»**, а не «осталось»: это то же число из той
> же функции, что показывают детали, панель каскада и settle-диалог, а оператор проходит все
> четыре поверхности в одном сценарии (замерено: 2 строки в ячейке ~150px в обоих вариантах,
> унификация бесплатна). (2) Вторая половина **не рендерится вовсе**, когда доплачивать нечего
> (`remaining === 0`): «к доплате 0,00» на каждой полностью закрытой строке — это строка, которую
> надо прочитать и отбросить. Изначальная формула §5.1 печатала её всегда.
...
> **Правка, 2026-08-25 (QA-H-1, manual QA).** Текст **разветвлён по состоянию строки** — прежняя
> единственная формулировка («…строка остаётся оплаченной») описывала два разных исхода и для
> одного из них была неверна:
>
> | Состояние обязательства | Что произойдёт                  | Хвост сообщения                                                     |
> | ----------------------- | ------------------------------- | ------------------------------------------------------------------- |
> | `PAID` (остаётся PAID)  | не пишется ничего               | `…строка остаётся оплаченной, разница сама не вернётся`             |
> | открытое (`PENDING`)    | сумма поднимается до накопителя | `…сумма останется на уровне выплаченного, разница сама не вернётся` |
>
> Второе состояние достижимо: закрыть обязательство полностью → поднять сумму источника (строка
> откатывается в `PENDING_PAYMENT`, сохраняя накопитель) → снизить обратно ниже выплаченного.
> Строка при этом `PENDING_PAYMENT` и до, и после сохранения — называть её «оплаченной» неверно. Дополнительно: строка-производная с этим warning'ом **не получает** `needsReconfirm`-
...
> **СТАТУС: реализовано, 2026-08-24 (UX-1 + UX-7).** `receiverName` (nullable) добавлен в
> `cascadeDerivativePlanSchema`, заполняется в `loadCascadeSnapshot` из `receiver.displayName`
> по явному allow-list `columns` (не `receiver: true` — иначе e-mail и юр. имя уехали бы в тело
> ответа). Обе ветки — синьора и дропа — читают его на экране; асимметрия, описанная ниже,
> устранена. Абзац «Факт» выше описывает состояние ДО этой правки и оставлен как история.

**Предложение (выполнено):** добавить `receiverId`/`receiverName` (nullable) в `cascadeDerivativePlanSchema`,
...
## 15. Допущения — раунды 6-7 (решения, принятые под запись)

Все четыре вызваны находками manual QA и security-review на живых данных; ни одно не было в
исходном задании.

- **Контракт производной расширен полем `recomputedShare`** (QA-H-1). Альтернатива — «просто
  прижать `newAmount` полом» — не рассматривалась как достаточная: пол **уничтожает** сырую долю,
  а её потребляют три места (текст `OVERPAYMENT`, предикат переплаты, поля `newShare`/`overpaidBy`
  в журнале `CASCADE_OVERPAYMENT`). Форма вынуждена природой информации.
  _Побочно, для протокола:_ первоначальное обоснование этого решения в комментарии утверждало, что
  одновеличинный вариант сломал бы журнал **молча**. Это неверно и было опровергнуто
  **экспериментом** (security-review): имитация одновеличинного фикса роняет **четыре**
  пред-существующих теста — один в `@crm/shared` (предупреждение о переплате перестаёт
  срабатывать: `expected [] to deeply equal [{ code: 'OVERPAYMENT' }]`) и три в `@crm/api`
  (ассерции журнала `CASCADE_OVERPAYMENT`). Падает громко, а не молча. Ручка приведена
  намеренно: без неё утверждение о четырёх тестах нельзя перепроверить, не повторив
  эксперимент целиком — что для записи, живущей месяцами, равносильно выводу без условия
  проверки. Три независимых прослеживания
  кода (кодер, code-review, spec-review) дали один и тот же неверный ответ, потому что все три
  проверяли логику сравнения, а не поведение системы. Комментарий переписан на настоящую причину.

- **Текст `OVERPAYMENT` разветвлён по состоянию обязательства** (QA-H-1, второй слой) — см. §6.

- **Пол при записи и пол в резолвере сведены к ОДНОМУ предикату** (SR-M-3). Запись флорила
  безусловно, а резолвер пропускает пол, когда накопитель несопоставим с записываемой долей
  (`max()` над 260 USD и долей 26 USDT — не большее число, а бессмысленное). Два похожих предиката
  расходились ровно на одной популяции — открытое обязательство с накопителем в другой валюте:
  план показывал 26, строка приняла бы 260. Теперь обе стороны вызывают `settledCurrencyMismatch`,
  одну и ту же экспортируемую функцию на тех же значениях: разойтись нечему.

  **Чего здесь сознательно НЕ сделано:** эта популяция намеренно проходит Phase 1. AC15 отказывает
  в **откате**, который потом нельзя будет закрыть; на открытом обязательстве откат не происходит,
  а обычное **увеличение** суммы такой строки записывается и должно продолжать записываться.
  Первая версия фикса расширяла отказ Phase 1 — и была поймана существующим тестом, который
  формулирует этот carve-out словами. Правка ушла туда, где расхождение и живёт, а не туда, где
  его проще всего было запретить.

- **Кнопка расчёта при нулевом остатке — «Закрыть без доплаты»** вместо «Отметить как оплачено».
  Атрибуция: **owner decision via оркестратор**, после того как QA дошёл до состояния живьём
  (кнопка рабочая, обязательство закрывается, баланс не двигается) и обе оси — дизайн и QA —
  независимо отметили, что подпись обещает перевод денег, которого не будет.
