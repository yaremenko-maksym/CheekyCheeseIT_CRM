...
| Состояние                                   | Проверено                          | Результат                                                                                            |
| ------------------------------------------- | ---------------------------------- | ---------------------------------------------------------------------------------------------------- |
| Первое добавление (`currentEmail=null`)     | live, JUNIOR без личного email     | плейсхолдер, кнопка золотая (не revokesExisting), описание диалога звучит не по месту (см. `UX-H-4`) |
...
`Findings: UX-H-2, UX-H-3, UX-M-2, UX-H-4 (4)` — UX-H-2/UX-H-3/UX-M-2 закрыты
(исправлены мной, перепроверены живьём); UX-H-4 открыта, передаю как находку
(текст↔поведение, не косметика apps/web).

## Handoff (раунд 2)

Косметические фиксы применены в моей зоне (`apps/web/**`), ESLint + typecheck

- `pnpm --filter @crm/web build` чисты, релевантные юнит-тесты (`user-profile`,
  `admin-actions`) зелёные. `UX-H-4` — не моя зона (текст диалога владеет
  copy-reviewer, реальная инвалидация сессии — backend); передаю через PM как
  самостоятельную находку, не блокирую этим фиксацию остального раунда.
