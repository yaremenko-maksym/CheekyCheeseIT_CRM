...
| **Hash-ссылка С ДРУГОЙ страницы** (юзер на `/careers`, кликает «Contact» в nav → уходит на `/#contact`) | Route МЕНЯЕТСЯ → полноценный page-transition (§M.3, scrim+caret-line). `hashScrollIntoView` роутера остаётся **default (true)** — как только scrim открывает новую страницу (scrim-out завершён), пользователь уже видит `/` заскроленным ровно к `#contact` (роутер это делает мгновенно на `onRendered`, ДО того как scrim открылся — см. ниже). **Дополнительный плавный скролл поверх НЕ проигрывается** — это была бы вторая анимация подряд («не бесить»), а не «премиум-сдержанно». |
...
4. **Тёмный scrim + тонкая жёлтая caret-line page-transition** (§M.3, HOTFIX 2026-07-24 —
   заменяет полноэкранную жёлтую заливку v1) — люминанс-безопасный (WCAG 2.3.1 расчёт §M.3.0),
   мягкий (`EASE_SOFT`, 500мс) переход между всеми роутами, облегчённый back-вариант.
