 ...
 *   - Archive button still appears on a SENIOR row in /crm/users
...
test.describe('Senior archive — pair-archive regression (AC7)', () => {
  test('archive button visible for the SENIOR row on /crm/users', async ({ asAdmin: page }) => {
  ...
  test('Confirm DELETE goes to /api/users/<senior-id> — legacy contract', async ({ asAdmin: page }) => {
    await page.goto('/crm/users')

    const deleteReq = page.waitForRequest(
      (req) =>
        req.url().includes(`/api/users/${USERS.senior.id}`) && req.method() === 'DELETE',
    )
  ...
  test('Drop entities (drop user row) untouched after archiving a senior', async ({ asAdmin: page }) => {
    await page.goto('/crm/users')
