...
// (non-breaking space) — a regular space would let "10,0" / "МБ" split
// across a line at 320px.
describe('formatBytes', () => {
  ...
  it('joins the number and unit with a non-breaking space (COPY-L-3)', () => {
    expect(formatBytes(1024, 'uk')).toContain(' ')
    expect(formatBytes(1024, 'uk')).not.toMatch(/\d /)
  })
})
