...
function TeamPage() {
  ...
  useEffect(() => {
    ...
    if (user.role === 'SENIOR' || user.role === 'JUNIOR' || user.role === 'DROP') {
      if (teams.length === 1 && teams[0]) {
        navigate({ to: '/team/$teamId', params: { teamId: teams[0].id } })
      }
    }
  ...
  // Precompute active-project count per team in a single O(N) pass so the sort
  // comparator never calls projects.filter() on every pair (was O(N²)).
  const activeProjectCountByTeam = useMemo<Map<string, number>>(() => {
    if (!projects || !teams) return new Map()
    const countMap = new Map<string, number>()
    for (const team of teams) {
      const seniorIds = new Set(
        team.members.filter((m) => m.role === 'SENIOR').map((m) => m.userId),
      )
      const count = projects.filter(
        (p) => p.archivedAt === null && seniorIds.has(p.seniorId ?? ''),
      ).length
      countMap.set(team.id, count)
    }
    return countMap
  }, [projects, teams])
  ...
  const filteredTeams = useMemo(() => {
    ...
    result.sort((a, b) => {
      ...
      if (sortBy === 'projects') {
        const aCount = activeProjectCountByTeam.get(a.id) ?? 0
        const bCount = activeProjectCountByTeam.get(b.id) ?? 0
    ...
    return result
  }, [teams, activeProjectCountByTeam, search, sortBy])
  ...
  const handleTeamTabChange = (next: TeamTab) => {
    ...
    startTransition(() => {
      if (next === 'ARCHIVED') {
        navigate({ to: '/team', search: { archived: true } })
        return
      }
      if (isArchivedView) {
        navigate({ to: '/team', search: {} })
      }
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      >
        {/* chrome-in-place: skeleton rows replace the full-page loading guard;
            StickyPageHeader + tabs above are always visible. */}
        {isLoading ? (
          ...
          <div className="space-y-6">
            ...
            >
              ...
              {filteredTeams.map((team) => {
                ...
                // Reuse precomputed Map (O(1) lookup) instead of filtering per card.
                const activeProjects = activeProjectCountByTeam.get(team.id) ?? 0
