...
export function InterviewDetailSheet({
  ...
  function FieldRow({ name, label, placeholder, schema, textarea = false }: {
      ...
      <SheetContent side="right" className="w-full sm:max-w-lg flex flex-col overflow-hidden">
        <SheetHeader className="mb-4 shrink-0">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <span className={cn('text-xs font-medium px-2 py-0.5 rounded-full border', STAGE_BADGE_COLORS[interview.stage])}>
              {STAGE_LABELS[interview.stage]}
            </span>
            {interview.seniorName && (
              <span className="text-xs text-muted-foreground">Senior: {interview.seniorName}</span>
            )}
          </div>
        </SheetHeader>
        ...
        <div className="flex flex-col gap-4">
          <div className="space-y-3">
            <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Основное</h3>
            <FieldRow
              name="companyName"
              label="Компания"
          ...
          {canMove && (!isTerminal || canMoveTerminal) && (
            <div className="space-y-2">
              <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Этап</h3>
          ...
          <div className="space-y-3">
            <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Заметки синьора</h3>
            ...
            <FieldRow name="notesTechStack" label="Стек технологий" placeholder="React, Node.js, AWS" schema={updateInterviewSchema.shape.notesTechStack.unwrap().unwrap()} />
            <FieldRow name="notesTeamSize" label="Состав команды" placeholder="5 devs, 2 QA, 1 PM" schema={updateInterviewSchema.shape.notesTeamSize.unwrap().unwrap()} />
            <FieldRow name="notesBenefits" label="Бенефиты" placeholder="20 days vacation, health insurance" schema={updateInterviewSchema.shape.notesBenefits.unwrap().unwrap()} />
            <FieldRow name="notesPaymentType" label="Тип оплаты" placeholder="ФОП / гиг-контракт / крипта" schema={updateInterviewSchema.shape.notesPaymentType.unwrap().unwrap()} />
            <FieldRow name="notesSalaryReview" label="Пересмотр зарплаты" placeholder="через 3 місяці" schema={updateInterviewSchema.shape.notesSalaryReview.unwrap().unwrap()} />
            <FieldRow name="notesCorpTech" label="Корпоративная техника" placeholder="MacBook Pro M3, iPhone 15..." schema={z.string().max(255)} />
            <FieldRow name="notesGeneral" label="Общие заметки" placeholder="Свободные заметки..." schema={updateInterviewSchema.shape.notesGeneral.unwrap().unwrap()} textarea />
          </div>
        ...
        <DialogFooter>
          ...
          <Button
            onClick={() => {
              ...
              if (createdProjectId) {
                void navigate({ to: '/crm/projects/$projectId', params: { projectId: createdProjectId } })
              }
            }}
          >
            Да, перейти
