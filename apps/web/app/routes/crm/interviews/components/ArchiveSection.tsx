  ...
  return (
    <div className="flex flex-col min-w-[180px] w-44 shrink-0">
      <div className="mb-3 flex items-center gap-2">
        <span className={cn('text-xs font-semibold px-2 py-0.5 rounded-full border', STAGE_BADGE_COLORS[stage as InterviewStage])}>
      ...
      <SortableContext items={cards.map((c) => c.id)} strategy={verticalListSortingStrategy}>
        <div
          ref={setNodeRef}
          className={cn('flex flex-col gap-2 min-h-10 rounded-lg p-1 transition-colors', isOver && 'bg-accent/40')}
        >
          {cards.map((card) => (
            <InterviewCard
              key={card.id}
