 ...
 * валидировано за месяц, выплачено за месяц, получателей. Главный CTA ведёт на
 * /crm/finance с предвыбранным PENDING-фильтром (deep-link `?status=PENDING`).
 *
...
export function AccountantDashboard() {
  ...
  const goValidatePending = () =>
    void navigate({ to: '/crm/finance', search: { status: 'PENDING' } })
    ...
    <main data-testid="accountant-dashboard-hub" className="space-y-6">
      ...
      {isLoading ? (
        ...
        <>
          ...
          {/* Primary CTA — validate pending payments (deep-link to PENDING filter). */}
