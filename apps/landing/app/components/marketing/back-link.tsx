 ...
 * lightweight page-transition variant instead of the primary yellow-wipe on
 * the next navigation, since the visitor is going somewhere they've already
 * seen. `markNextTransitionLight()` runs synchronously in `onClick` — React
 * invokes a DOM node's own `onClick` prop before `<Link>`'s internal handler
 * fires on the SAME element, so this always wins the race against
