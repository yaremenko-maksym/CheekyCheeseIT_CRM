...
describe.skipIf(!hasDatabaseUrl())(
  'payout settlement — NO redundant 50/50 PAYOUT_ADMIN split (real DB)',
  () => {
    ...
    beforeAll(async () => {
      try {
        ...
        await probe.end()
        if (check.rowCount === 0) {
          throw new Error('[payout-no-admin-split] FAILED — payout_requests table not found')
        }
      } catch {
        throw new Error('[payout-no-admin-split] FAILED — no DB reachable at DATABASE_URL')
      }
