...
describe.skipIf(!hasDatabaseUrl())(
  'payout → company wallet — on-chain confirm + manual-confirm RBAC (real DB)',
  () => {
    ...
    beforeAll(async () => {
      try {
        ...
        await probe.end()
        if (check.rowCount === 0) {
          throw new Error('[payout-company-wallet] FAILED — payout_requests table not found')
        }
      } catch {
        throw new Error('[payout-company-wallet] FAILED — no DB reachable at DATABASE_URL')
      }
    ...
    it('RBAC: the recorded sender is ADMIN/ACCOUNTANT-only in the DTO', async () => {
      ...
      expect((await svc.findPayoutRequest(requestId, ACCOUNTANT)).txFromAddress).toBe(
        EXCHANGE_WALLET,
      )
