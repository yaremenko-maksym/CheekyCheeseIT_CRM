...
describe.skipIf(!hasDatabaseUrl())(
  'drop payout → company account + senior obligation (real DB)',
  () => {
    ...
    beforeAll(async () => {
      try {
        ...
        await probe.end()
        if (check.rowCount === 0) {
          throw new Error('[drop-payout-company-account] FAILED — pending_obligations not found')
        }
      } catch {
        throw new Error('[drop-payout-company-account] FAILED — no DB reachable at DATABASE_URL')
      }
