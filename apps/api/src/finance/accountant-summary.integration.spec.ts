...
describe.skipIf(!hasDatabaseUrl())(
  'accountant-summary — real backend integration (real DB, no mocks)',
  () => {
    ...
    beforeAll(async () => {
      try {
        ...
        if (schemaCheck.rowCount === 0) {
          throw new Error('[accountant-summary integration] FAILED — transactions table not found')
        }
      } catch {
        throw new Error(
          '[accountant-summary integration] FAILED — no DB reachable at DATABASE_URL (expected in CI unit job)',
        )
      }
