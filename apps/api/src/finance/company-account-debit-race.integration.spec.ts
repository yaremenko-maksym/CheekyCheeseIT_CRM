...
describe.skipIf(!hasDatabaseUrl())(
  'company-account debits — serialized via advisory lock (MED-1, real DB)',
  () => {
    ...
    beforeAll(async () => {
      try {
        ...
        await probe.end()
        if (check.rowCount === 0) {
          throw new Error('[company-account-debit-race] FAILED — funding_source column not found')
        }
      } catch {
        throw new Error('[company-account-debit-race] FAILED — no DB reachable at DATABASE_URL')
      }
