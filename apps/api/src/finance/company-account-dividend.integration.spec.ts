...
describe.skipIf(!hasDatabaseUrl())(
  'createDividend — balance gate + advisory lock (MED, real DB)',
  () => {
    ...
    beforeAll(async () => {
      try {
        ...
        await probe.end()
        if (check.rowCount === 0) {
          throw new Error('[company-account-dividend] FAILED — funding_source column not found')
        }
      } catch {
        throw new Error('[company-account-dividend] FAILED — no DB reachable at DATABASE_URL')
      }
