...
describe.skipIf(!hasDatabaseUrl())(
  'company-account ledger + reconciliation (real DB, no mocks)',
  () => {
    ...
    async function myContribution(): Promise<number> {
      const sumByType = async (type: string, companyFundedOnly: boolean): Promise<number> => {
        const conds = [
          ...
          ...(companyFundedOnly
            ? [eq(transactions.fundingSource, 'COMPANY_ACCOUNT' as never)]
            : []),
        ]
    ...
    beforeAll(async () => {
      try {
        ...
        if (check.rowCount === 0 || col.rowCount === 0) {
          throw new Error(
            '[company-account-ledger] FAILED — schema not present (company_account / funding_source)',
          )
        }
      } catch {
        throw new Error('[company-account-ledger] FAILED — no DB reachable at DATABASE_URL')
      }
