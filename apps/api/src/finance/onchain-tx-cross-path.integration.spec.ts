...
// 'COMPANY_ACCOUNT'` toggle is replaced by `receiverId` (same sentinel
// `declareUsdtProjectIncome` already used) — every company-funded call below
// is updated to match, preserving the exact same routing this spec pins.
...
describe.skipIf(!hasDatabaseUrl())(
  'on-chain hash consumption is CROSS-PATH (payout ⟷ deposit, real DB)',
  () => {
    ...
    beforeAll(async () => {
      try {
        ...
        await probe.end()
        if (check.rowCount === 0) {
          throw new Error('[onchain-tx cross-path] FAILED — consumed_tx_hashes table not found')
        }
      } catch {
        throw new Error('[onchain-tx cross-path] FAILED — no DB reachable at DATABASE_URL')
      }
