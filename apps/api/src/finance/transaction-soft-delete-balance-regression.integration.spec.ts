...
describe('task-soft-delete-and-money-audit — balance regression (real-DB, AC4)', () => {
  beforeAll(async () => {
    try {
      ...
      console.warn(
        '[soft-delete-balance-regression] SKIPPED — no DB reachable at DATABASE_URL (expected in CI unit job)',
      )
      dbAvailable = false
      return
    }
  ...
  it('getSeniorSummary.seniorShareIncome — insert + soft-delete a PAID SENIOR_INCOME nets to baseline', async () => {
    ...
    expect(afterInsert.seniorShareIncome.total).toBeCloseTo(before.seniorShareIncome.total + 260, 6)
