...
describe('security-review PR #456 round 2 (MED-1) — cross-module visibility regression (real-DB)', () => {
  beforeAll(async () => {
    try {
      ...
      console.warn(
        '[soft-delete-cross-module-visibility] SKIPPED — no DB reachable at DATABASE_URL (expected in CI unit job)',
      )
      dbAvailable = false
      return
    }
  ...
  describe('HIGH-1 — InvoicesService, real deleted-transaction proof', () => {
    ...
    it('after adminDeleteTransaction: counterparty gets 404 from getInvoice; ADMIN still gets 200', async () => {
      ...
      const svc = makeInvoicesSvc()
      await expect(svc.getInvoice(SENIOR_1, SENIOR_INCOME_TX_ID)).rejects.toThrow(NotFoundException)
  ...
  describe('HIGH-2 — CompanyAccountService.getDepositStatus, real deleted-transaction proof', () => {
    ...
    it('after adminDeleteTransaction: owner gets 404 (was a normal 200 before HIGH-2 was fixed)', async () => {
      ...
      const svc = makeCompanyAccountSvc()
      await expect(svc.getDepositStatus(DEPOSIT_TX_ID, SENIOR_1)).rejects.toThrow(NotFoundException)
    })
