...
describe.skipIf(!hasDatabaseUrl())(
  'DROP obligation settle — currency conversion (real DB, task-drop-payout-currency)',
  () => {
    ...
    beforeAll(async () => {
      try {
        ...
        await probe.end()
        if (check.rowCount === 0) {
          throw new Error('[drop-payout-currency] FAILED — pending_obligations not found')
        }
      } catch {
        throw new Error('[drop-payout-currency] FAILED — no DB reachable at DATABASE_URL')
      }
