...
describe('UsersService.buildProfileView — ACCOUNTANT requisites audit + wallet exclusion (real DB)', () => {
  beforeAll(async () => {
    try {
      ...
      console.warn('[accountant-requisites-audit] SKIPPED — no DB at DATABASE_URL')
      dbAvailable = false
      return
    }
  ...
  it('AC1a: ACCOUNTANT reading a SENIOR writes ONE requisites_read audit (redacted)', async () => {
    ...
    const result = await service.buildProfileView(viewerOf(ACCOUNTANT_ID, 'ACCOUNTANT'), SENIOR_ID)
