 ...
 * hrjoin-. DB-SKIP-GUARD: dbAvailable=false when DATABASE_URL unreachable.
 */
...
describe('MED-3 (security-review round 2): HR createUser teamMode=JOIN_DROP_TEAM scope (real DB)', () => {
  beforeAll(async () => {
    try {
      ...
      console.warn('[hr-join-drop-team-scope] SKIPPED — no DB at DATABASE_URL')
      dbAvailable = false
      return
    }
