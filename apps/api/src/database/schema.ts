...
export const transactions = pgTable(
  'transactions',
  {
     ...
     *   CREATE UNIQUE INDEX uq_transactions_senior_income_idempotency_key
     *     ON transactions (idempotency_key)
     *     WHERE type = 'SENIOR_INCOME' AND idempotency_key IS NOT NULL;
     *   CREATE UNIQUE INDEX uq_transactions_drop_income_idempotency_key
     *     ON transactions (idempotency_key)
  ...
  (t) => [
    ...
    // backlog 73/A-3: idempotency key for SENIOR_INCOME (createSeniorIncome).
    // Same 1:1 copy of the ADMIN_INCOME contract above — a double-click / retry
    // that slips past the service's early-SELECT collides here (23505) instead
    // of creating a second SENIOR_INCOME row, which would otherwise book a
...
export const notifications = pgTable(
  'notifications',
  {
    ...
    //
    // Имя колонки здесь совпадает с именем свойства, а drizzle при ПУСТОМ
    // имени берёт имя свойства. Проверено исполнением на нашей версии:
    // `jsonb('')` даёт колонку `data`, тогда как `varchar('')` в соседнем
    // `dedupeKey` дало бы `dedupeKey` вместо `dedupe_key` — потому там мутант
    // и погибает от проверки имён, а здесь погибнуть не может ни от какого
    // утверждения: мутация не меняет ничего наблюдаемого.
    // Stryker disable next-line StringLiteral: drizzle подставляет имя свойства вместо пустого — колонка остаётся `data`
    data: jsonb('data'),
