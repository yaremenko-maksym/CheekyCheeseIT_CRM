 ...
 *   • An in-memory Map<userId, {role, expiresAt}> cache (TTL: CACHE_TTL_MS)
 *     prevents a DB hit on every request. The cache is per-guard-instance
 *     (singleton in NestJS DI), so it is shared across all requests in the
 *     same process. TTL is intentionally short (60 s) to bound the revocation
 *     lag while avoiding per-request DB overhead.
 *
 * Trade-offs (AC2):
 *   • Revocation lag = CACHE_TTL_MS (60 s). An admin who demotes a user will
 *     see the new role enforced within 60 s — intentional design choice.
...
export class JwtAuthGuard implements CanActivate {
  /** Short-lived in-memory cache: userId → { role, expiresAt } */
   ...
   * Caches the result for CACHE_TTL_MS to avoid per-request DB queries.
   * Throws UnauthorizedException if the user is not found or archived.
   *
  ...
  private async resolveCurrentUser(jwtUser: JwtPayload): Promise<JwtPayload> {
    ...
    if (dbUser.archivedAt) {
      ...
      // Evict any stale cache entry so the rejection is not cached.
      this.userCache.delete(jwtUser.id)
      throw new UnauthorizedException()
    }

    // Populate cache with fresh role.
