...
describe.skipIf(!hasDatabaseUrl())(
  'Projects JUNIOR allowlist-masking — real DB integration',
  () => {
    ...
    beforeAll(async () => {
      ...
      try {
        ...
        throw new Error(
          '[projects-junior-masking integration] FAILED — no DB at DATABASE_URL (expected in CI unit job)',
        )
      }
...
describe.skipIf(!hasDatabaseUrl())(
  'Legend persona enrichment + SENIOR isolation — real DB integration',
  () => {
    let app: NestFastifyApplication
    let jwt: JwtService
    let dbSvc: DatabaseService
