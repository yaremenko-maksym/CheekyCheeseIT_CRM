...
// ── Suite ───────────────────────────────────────────────────────────────────
describe('Credentials RBAC — real backend integration (real DB, no mocks)', () => {
  ...
  beforeAll(async () => {
    try {
      ...
      if (schemaCheck.rowCount === 0) {
        console.warn(
          '[credentials-rbac integration] SKIPPED — project_credentials table not found ' +
      ...
      console.warn(
        '[credentials-rbac integration] SKIPPED — no DB reachable at DATABASE_URL (expected in CI unit job)',
      )
      dbAvailable = false
      return
    }
