...
describe('createInTx ставит письмо в очередь', () => {
  ...
  it('старому типу строка заводится SKIPPED/LEGACY_TYPE, а не пропускается молча', async () => {
    // SPEC-H-1: у типа вне реестра письма нет ни шаблона, ни настройки — но
    // след «письма не полагалось» нужен ровно так же, как всем остальным
    // причинам.
    //
    ...
    await h.service.createInTx(h.tx as never, {
      userId: 'u-1',
      type: 'SOME_TYPE_OUTSIDE_THE_REGISTRY',
