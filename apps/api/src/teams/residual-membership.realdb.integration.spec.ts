...
describe.skipIf(!hasDatabaseUrl())(
  'MED-2 (security-review round 2): residual leftAt-filter gaps (real DB)',
  () => {
    beforeAll(async () => {
      try {
        ...
        throw new Error('[residual-membership] FAILED — no DB at DATABASE_URL')
      }
    ...
    it('AC-I (MED-1, round 4): rotateSenior itself writes the positive team_member_removed/SENIOR evidence wasFormerMemberOfTeam now requires', async () => {
      ...
      const rows = await db
        .select()
        .from(teamAuditLog)
        .where(eq(teamAuditLog.targetId, DROP_TEAM_ID))
