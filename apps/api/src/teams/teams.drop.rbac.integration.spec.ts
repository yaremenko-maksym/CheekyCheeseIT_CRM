...
describe('Teams DROP RBAC — real backend integration (real DB, no mocks)', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      console.warn(
        '[teams-drop-rbac integration] SKIPPED — no DB reachable at DATABASE_URL (expected in CI unit job)',
      )
      dbAvailable = false
      return
    }
