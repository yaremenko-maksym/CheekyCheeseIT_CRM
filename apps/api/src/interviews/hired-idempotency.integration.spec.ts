...
describe('BIZ-07 — HIRED idempotency: repeated move to HIRED must not duplicate projects', () => {
  let pool: Pool
  ...
  beforeAll(async () => {
    try {
      ...
      if (col.rowCount === 0) {
        console.warn(
          '[hired-idempotency integration] SKIPPED — created_project_id column not yet migrated',
        )
        dbAvailable = false
        return
      }
    } catch {
      console.warn('[hired-idempotency integration] SKIPPED — no DB reachable at DATABASE_URL')
      dbAvailable = false
      return
    }
  ...
  it('created_project_id has a partial unique index — DB rejects second non-null value for same interview', async () => {
    ...
    await svc.move(CARD_A_ID, { stage: 'HIRED', position: 0 }, ADMIN)
    const ivRow = await dbSvc.db.query.interviews.findFirst({ where: eq(interviews.id, CARD_A_ID) })
