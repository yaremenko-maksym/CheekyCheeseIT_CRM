...
describe('Interviews move() — transactional integrity (real DB, no mocks)', () => {
  let pool: Pool
  ...
  beforeAll(async () => {
    try {
      ...
      console.warn(
        '[interviews-move-transaction integration] SKIPPED — no DB reachable at DATABASE_URL (expected in CI unit job)',
      )
      dbAvailable = false
      return
    }
