...
2. `docs/agents/<agent>.md` — per-agent workflow / golden rules / mandatory tables.
3. `rules/common/*.md` — cross-cutting standards (этот файл и соседи).

## Anti-patterns

- **Mandatory table в `<agent>.md` без актуального trigger** — skill становится "discoverable in theory" но never invoked. PM при review агентов проверяет: `grep skill-name docs/agents/*.md`.
...
- `rules/common/mcp-first.md` — MCP catalog (некоторые skills используют MCP tools).
- `rules/common/zone-of-write.md` — какие skills доступны кому (per-agent invocation).
