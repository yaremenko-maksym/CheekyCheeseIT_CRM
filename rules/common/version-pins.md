...
- `rules/common/git-policy.md` — изменение pins требует отдельного commit с `vision:` для CI matrix.
- `rules/common/zone-of-write.md` — `package.json` overrides — DevOps зона (не Coder).

## Источники

- CLAUDE.md "Ключевые ограничения версий" + "Ключевые технические заметки".
- `docs/agents/project-state.md` — авторитетный snapshot версий.
