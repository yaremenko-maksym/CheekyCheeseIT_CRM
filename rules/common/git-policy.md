...
| `git add .` / `git add -A` / `git add *` / `git add apps/`       | Подметает чужие debug-артефакты из worktree (PR #22 round4 incident, см. `docs/agents/memory/coder/lessons.md` 2026-05-20 [P0]). | Только явный список файлов из task-секции «Конкретные изменения». |
...
Pre-push hook (`.claude/hooks-ecc/coder-pre-push.sh`) блокирует `git push` если последний commit на ветке `feature/*` / `fix/*` / `infra/*` / `test/*` не содержит `ac_verified:`. Не обходить — доделать AC.
...
Стандартные: `feat`, `fix`, `refactor`, `docs`, `test`, `chore`, `perf`, `ci`. Project-scopes: `(api)`, `(web)`, `(shared)`, `(coder)`, `(architect)`, `(pm)`, `(devops)`, `(autotest)`, `(legal)`, `(reviewer)`. Commit message body — на английском (Conventional Commits standard). User-facing assistant ответы — на русском (см. `rules/common/russian-language.md`).

## Связанные правила

- `rules/common/zone-of-write.md` — какой агент может писать какие пути (Reviewer выдаёт BLOCK на нарушения).
- `rules/common/russian-language.md` — assistant outputs русский, commits английский.
...
- `docs/agents/memory/coder/lessons.md` 2026-05-20 [P0] git-add zero-tolerance
- `docs/agents/memory/coder/lessons.md` 2026-06-02 [P0] `--no-verify` recurrence
