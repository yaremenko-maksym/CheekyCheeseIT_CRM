...
- После каждого Edit / Write на `.ts` / `.tsx` → `eslint lint-files` вместо ожидания pre-commit хука. Подробности — `rules/common/eslint-mcp-first.md`.
...
- `rules/common/eslint-mcp-first.md` — детали ESLint MCP замены post-edit hook'а.
- Superpowers skills invocation — `rules/common/skills-invocation.md`.
