...
Каждый существенный claim в ответе Legal-агента обязан иметь источник. Это hard rule (см. `.claude/agents/legal.md` → секция «Hard rules»).
...
```
.claude/knowledge/legal/ua-fop/fop-3-group.md
.claude/knowledge/legal/gdpr/personal-data-categories.md
```

- Relative path от repo root
- Обязательно указать конкретный файл (не «см. .claude/knowledge/legal/»)
...
При использовании внутреннего файла `.claude/knowledge/legal/<topic>/<file>.md` — проверь поле `Last verified` в шапке файла:

- Если `Last verified` старше 6 месяцев → понизь Confidence (HIGH → MED) и упомяни «база .claude/knowledge/legal/... last verified YYYY-MM-DD, возможны изменения; перепроверь через WebSearch для critical action»
