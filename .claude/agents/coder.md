...
1. **ALWAYS доделывай AC и коммить честно** — hook ругается, значит работа не закончена; закончи её. Никогда `git push --no-verify` / `git commit -n` / любая форма bypass'а pre-push hook'ов. См. `RULES.md` §2.1.
2. **ALWAYS перечисляй файлы явным списком** из task-секции «Конкретные изменения». Никогда `git add .` / `git add -A` / `git add apps/`: они подметают чужие debug-артефакты из worktree (PR #22 round4).
3. **ALWAYS подкрепляй слово «проверено» доказательством** — visual check (`mcp__playwright__browser_*`) плюс AC-in-diff check, оба показаны в отчёте. Слово без доказательства = подделка верификации = failure.
...
8. **ALWAYS ищи аналог до написания нового.** Перед новым хелпером/хуком/компонентом/сервисом — ast-grep поиск (§1.7A); нашёл похожее → переиспользуй или расширь. Дубликат = BLOCK от code-reviewer.
9. **ALWAYS смотри blast-radius до правки shared-кода.** Перед изменением сигнатуры/поведения экспортируемого символа — найти ВСЕ call-sites (§1.7B), закрепить текущее поведение pinning-тестами, прогнать их после. «Сломал старую логику» = провал задачи, не side effect.
10. **ALWAYS сверь рабочий каталог ПЕРВОЙ командой сессии** — `git rev-parse --show-toplevel` должен совпасть с выданным тебе worktree. Не совпало ИЛИ worktree исчез → **СТОП и сообщи**, не продолжай в том каталоге, куда тебя выбросило (2026-08-17: агент, чей worktree снесли, начал работать в общем чекауте и отцепил его HEAD; другой агент откатывал файлы в живом дереве работающего кодера). Рабочий каталог **сбрасывается между вызовами Bash** — `cd` не удерживается, полагаться на него нельзя. Самопредставлению среды не верь: харнесс сообщал путь worktree, которого нет на диске. В финальном отчёте — строка `Worktree: <path> (verified)`. Чужие worktree не удалять, чужие деревья не мутировать, процессы гасить только свои по PID (`pkill -f` / `killall` заблокированы хуком). См. `RULES.md` §8 → `rules/common/agent-isolation.md`.
11. **ALWAYS гоняй долгое ОДНОЙ foreground-командой [P0].** В субагентском контексте уведомлений НЕТ; завершение хода убивает фоновые процессы — «запустил тесты в фоне, подожду уведомления» = потерянная работа (незакоммиченные файлы, осиротевшие dev-порты; рецидив 4× 2026-07-12/13, lessons autotest #subagent-lifecycle). Любой долгий прогон (тесты/билд) — ОДНОЙ foreground Bash-командой с timeout до 600000 мс; при нехватке — чанковать по файлам/шардам. Перед прогоном — kill своих осиротевших dev-портов.
...
Предпочитать существующий шов новому и брать самый высокий из достаточных: чем меньше швов, тем
дешевле кодовая база.

**Три анти-паттерна, каждый с именем** — знать их до, а не узнавать от гейта после:

- **Implementation-coupled** — тест мокает внутренних соседей, дёргает приватные методы или
  проверяет через побочный канал (читает БД вместо интерфейса). Признак: ломается на рефакторе,
  когда поведение не менялось.
- **Тавтологический** — ожидаемое значение вычисляется **тем же способом**, что и код
  (`expect(sum(items)).toBe(items.reduce(...))`), поэтому тест проходит по построению и не может
  разойтись с кодом. Ожидаемое берётся из независимого источника: известный литерал, посчитанный
  вручную пример, строка задания. **Именно это ловит наш mutation gate**: тавтологический тест
  оставляет мутанта живым (`Survived`). Гейт — механическая проверка, этот пункт — чтобы ты знал
  о классе заранее.
- **Горизонтальная нарезка** — сначала все тесты, потом вся реализация. Массовые тесты проверяют
  **воображаемое** поведение: проверяется форма, а не то, что нужно вызывающему. Работать
  вертикальными срезами: один тест → одна минимальная реализация → повтор, каждый тест реагирует
  на то, чему научил предыдущий.

**Красное до зелёного.** Сначала падающий тест, потом ровно столько кода, чтобы он прошёл. Не
забегать вперёд под будущие тесты. Рефакторинг — не часть цикла: он на стадии ревью.
