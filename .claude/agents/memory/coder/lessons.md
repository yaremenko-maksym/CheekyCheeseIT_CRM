...
2026-07-14 [P1] (#e2e-env #scratch-stack) E2E на скретч-стеке: ВСЕГДА задавать `E2E_REAL_API_BASE` (fixtures.ts) + `THROTTLE_RELAXED=true` (конвенция e2e.yml) — иначе фикстуры молча бьют в чужой live :3001. Глобальные env шелла (FRONTEND_URL/API_PORT/DATABASE_URL) НЕ перезаписываются dotenv — передавать env явно при старте стека (PR #367).
