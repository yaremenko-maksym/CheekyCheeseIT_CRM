...
- **Легаси-эпоха закрывается дизъюнктом, а не выяснением, запускался ли скрипт** (SR-H-3) —
  `apps/api/drizzle/manual/2026-07-15_settle_phantom_cleanup.sql` не завайрен в `deploy.yml`, и
  факт его прогона на проде нигде не записан. Предикат AC13 сделан верным в обеих эпохах при
  любом ответе: `closing_transaction_id = <id> AND status = 'PAID'` рядом с существующим ключом
  по `source_transaction_id`. Обратимо, откат: снять дизъюнкт (и вместе с ним — защиту до-июльских
  строк).
