...
- **BIZ-18 снимать хирургически — только для `amount`.** Широкое снятие («PAID-строка стала
  редактируемой») откроет живую ветку `data.currency` и зеркальный дефект 1.2/1.4.

---

## Часть 3. Открытые PR

- **#600** (инвойс, задача 4) — security-review дал `APPROVE` в раунде 6; идёт раунд 7 по хвостам
  (MED-G: флаг `mixedCurrency` утверждает `false` там, где не определялся; MED-H: неверное
  утверждение о покрытии в сообщении коммита; MED-I: устаревший JSDoc; LOW: `salaryMonth` из
  неупорядоченного массива). Метка `do-not-merge` — снимать после раунда.
- **#603** (задача 2) — code-review `APPROVE`, security-review `BLOCK`; идёт фикс-раунд 2
  (HIGH-2-residual + MED-A). Метка `do-not-merge`.
- **#602** — не наша, ведётся параллельной сессией (`infra/afk-pipeline-migration`). **Не трогать.**
