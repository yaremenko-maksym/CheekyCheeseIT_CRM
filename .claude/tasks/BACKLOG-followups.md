...
Чинить в сиде: добавить `company_account` в список усекаемых таблиц (либо явно обосновать, почему
она исключена — если исключена намеренно, это должно быть написано там же).
...
**112. Число в долгоживущей записи без ручки для перепроверки (§15 спеки предпросмотра).**
Найдено copy-review на #611 раунд 7 (COPY-L-8, LOW). Мержу не мешало, закрыто не было.
...
(swagger / openapi) нет. Финансовые роуты неавторизованному отвечают `404` — тем же кодом, что и
несуществующий путь, поэтому проба «есть ли эндпоинт» ничего не различает (проверено: заведомо
существующий список транзакций тоже даёт `404`). Войти нельзя: прод только через SSO.
