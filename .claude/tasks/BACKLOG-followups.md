...
**129. Убрать поле «Telegram-канал команды» — ссылку на канал так передать нельзя.**
Запрос владельца 2026-09-01, со снимком экрана формы.

Форма создания и профиль показывают поле, собирающее адрес вида `t.me/<имя>` с подписью
«Опционально. Канал для общения команды». Владелец: **в Telegram нельзя передать ссылку на
канал этим способом** — то есть поле собирает то, чем нельзя воспользоваться.

**Почему поле неверно, а не просто неудобно.** У команд компании каналы **приватные**
(подтверждено владельцем 2026-09-01), а у приватного канала адреса вида `t.me/<имя>` не
существует вовсе — туда попадают только по приглашению с хешем. Поле собирало формат, которого
для наших каналов физически не бывает.

**Решение владельца окончательное: убирать в любом виде.** Не менять формат на приглашение,
не прятать под условием — удалять. Спрашивать не нужно; причина записана выше именно затем,
чтобы через полгода не предложили «просто поменять формат» и не потратили на это раунд.

**Что удалять** (символ `telegramChannel`, 13 мест, из них 8 не тестовые): колонка в схеме,
контракты в общем пакете (команды и пользователи), сервисы и контроллеры команд и
пользователей, поле в диалоге создания пользователя.

**Порядок важен.** Сначала снять с интерфейса и из контрактов, отдельным шагом — колонку из
базы, и только после проверки, что в ней нет нужных данных. Удаление колонки необратимо, а
прод-миграции у нас катятся только через конвейер деплоя.

Проверить перед удалением колонки:

```sql
SELECT count(*) FILTER (WHERE telegram_channel IS NOT NULL) AS filled, count(*) AS total FROM teams;
```

Если заполненные строки есть — сначала решить с владельцем, что с ними делать: выгрузить,
перенести в другое поле или потерять сознательно.
