...
**Резюме:** Node 22 LTS, pnpm 7.32.4, Vite ^6.4 (НЕ 7.x), TanStack Router `1.170.15` + plugin `1.168.18` (peer-matched EXACT пара, номера НЕ совпадают), Tailwind v4, NestJS 11, Fastify ^5.8.5 через pnpm.overrides, Zod v4, PostgreSQL 16-alpine, Redis 7-alpine.
...
- **Agent isolation (у каждого агента своё рабочее пространство)** — **[`rules/common/agent-isolation.md`](rules/common/agent-isolation.md)** (2026-08-17). Каталог сессии выводится из идентификатора самого агента, не наследуется от оркестратора: рабочий каталог сбрасывается между вызовами Bash, поэтому `cd` не спасает. Пишущий агент — всегда `isolation="worktree"`; read-only — без worktree. Чужое дерево не мутировать, чужой worktree не удалять, `pkill -f` / `killall` запрещены. **Первой командой — `git rev-parse --show-toplevel` и сверка с выданным путём; не совпало или worktree исчез → СТОП и сообщить.** Хуки `pre:bash:cross-agent-blast` + `pre:agent:dispatch-isolation`; смоук `bash .claude/hooks/tests/cross-agent-hooks-smoke.sh`.
- **Review findings transfer (находки ревью не теряются по дороге)** — **[`rules/common/review-findings-transfer.md`](rules/common/review-findings-transfer.md)** (2026-08-17). Ревьюер нумерует находки (`CR-H-1`, `SR-M-2`) + контрольная строка `Findings: … (N)`; оркестратор переносит весь список; исполнитель отчитывается по каждому идентификатору, включая «не делал, потому что…».
- **Уровни автономии (сколько человека нужно решению)** — **[`rules/common/autonomy-levels.md`](rules/common/autonomy-levels.md)** (2026-08-22). Факты агент добывает сам и спрашивать их запрещено; решения классифицируются: **A1** обратимое — решить и записать строкой в `## Допущения`; **A2** — накопить в decision brief и спросить одной пачкой с дедлайном авто-принятия; **A3** необратимое и блокирующее — стоп, но встаёт только ветка, а не сессия. Critical-path zones автоматически дают A3. Мерж по-прежнему только по явному «мерджим».
- **Долговечность долгоживущих записей** — **[`rules/common/doc-durability.md`](rules/common/doc-durability.md)** (2026-08-22). Запись со сроком жизни > одного дня (бэклог, `.out-of-scope/`, ADR, lessons) описывает поведение и **символы**, никогда номера строк. Task-файлы и dispatch-промпты исключены намеренно: их читают в тот же день, и coordinator-дисциплина требует координат.
- **Фазовые границы** — **[`rules/common/phase-boundaries.md`](rules/common/phase-boundaries.md)** (2026-08-22). Шесть вариантов на границе фазы сверху вниз, первое «да» побеждает: продолжить → `/clear` → handoff → сабагент → внешний планировщик → компакт. Компакт внизу намеренно: всё, кроме «продолжить», превращает первоисточник во вторичный.
- **Mutation gate ↔ integration specs (`NoCoverage` там — не дыра)** — **[`rules/common/mutation-gate-integration-specs.md`](rules/common/mutation-gate-integration-specs.md)** (2026-08-18). Гейт гоняет только unit-набор; `*.integration.spec.ts` структурно исключены из discovery (`vitest.config.mts`) — `NoCoverage` на файле, который дергает только интеграционный спек, ожидаем. Покрытие для гейта дают unit-двойники — это требование инструмента, не дублирование. `NoCoverage` серьёзнее `Survived` (не исполнялось вовсе vs исполнялось и не поймано), кроме этого исключения.
