 ...
 * читатель её не увидел: «кто — по какому объекту» занимало оба ряда
 * `line-clamp-2`, а причина жила на третьем, которого нет.
 *
 * Число ИЗМЕРЕНО на живом стенде, а не прикинуто (первая редакция круга 5
 * ставила 30 на глаз — и на собственном скриншоте 320 цитата всё равно
 * переносилась на второй ряд, унося закрывающую «ёлочку»). Замер в попапе на
 * 320 px: колонка подробностей — 202 px, шрифт `12px Inter`; самый широкий
 * строчный кириллический глиф — 10.13 px (19 знаков в ряд), средний по
 * алфавиту — 7.32 px (27 знаков). Взято 22: обычная русская фраза (≈161 px)
 * укладывается с запасом в 40 px, то есть строка не переносится и закрывающая
 * кавычка остаётся видимой.
 *
 * Остаточный риск назван честно: цитата, набранная ТОЛЬКО самыми широкими
 * глифами («шшшшшшшшшшшшшшшшшшшшшш» ≈ 223 px), в ряд не уложится. Точного
 * ответа здесь быть не может — ширину знает браузер, а не разбор строки;
 * ровно поэтому граница проверяется E2E-тестом на настоящем рендере 320 px,
 * а не только арифметикой.
 */
...
export function describeNotification<T extends NewNotificationType>(
  ...
  switch (type) {
    ...
    case 'TRANSACTION_STATUS_CHANGED': {
      ...
      //
      // Порядок — ТОТ ЖЕ, что у `APPROVAL_REJECTED`: та же функция, тот же двухрядный
      // клип, та же семья сообщений. Обе раскладки на 320 px помещаются (промер
      // copy-review круга 4: ряд фактов — 152 px из 202 на четырёхзначной сумме,
      // переносится только на двенадцати разрядах), поэтому выбор сделан по единству
      // с соседом, а не по вместимости. `NOTIFICATION_DETAIL_LINE_CHARS` — бюджет
      // цитаты под худший кириллический глиф, а НЕ ширина ряда: применять его к
      // строке из цифр и латиницы нельзя.
 ...
 * `NotificationSubjectType` здесь не нужно — «Решение больше не требуется»
 * и «Решение уже принято» читаются одинаково что для проекта, что для доли
 * по пользователю.
 */
/**
...
export function notificationActions(n: RenderableNotification): NotificationAction[] {
  ...
  // того, как вообще посмотрит на согласование), а не «Решение больше не
  // требуется»: подпись описывает то, что заметнее и вернее — сам объект.
  //
