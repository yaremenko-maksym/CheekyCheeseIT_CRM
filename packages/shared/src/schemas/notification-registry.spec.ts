...
describe('describeNotification — все ветки, чтобы гейт мутаций видел каждую', () => {
  it.each([
    ...
    [
      ...
      // обрывок слова. Порядок (цитата первой) — ТОТ ЖЕ, что у
      // `APPROVAL_REJECTED`: обе раскладки на 320 px помещаются (промер
      // copy-review круга 4, см. тест ниже), выбор — по единству с соседом,
      // а не по вместимости.
      '«Нет чека»\nОтклонён: 10,00 USD',
    ],
